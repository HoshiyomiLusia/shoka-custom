...
const postBeauty = function () {
  ...
  $('.post.block').oncopy = function(event) {
    ...
    if(window.getSelection().toString().length > 999999999 && copyright) {
      event.preventDefault();
    ...
    instantsearch.widgets.hits({
      ...
      templates: {
        item: function(data) {
          ...
          // 2. 获取摘要 (新增逻辑)
          var snippet = '';
          if (data._snippetResult && data._snippetResult.contentStripTruncate) {
            snippet = '<p class="search-result-snippet" style="font-size: 0.9em; color: #888; margin: 5px 0 10px 0; line-height: 1.5; border-bottom: 1px dashed #ccc; padding-bottom: 5px;">' + 
              data._snippetResult.contentStripTruncate.value + 
              '</p>';
          }
          // 3. 返回：标题链接 + 摘要
          return '<a href="' + CONFIG.root + data.path +'">'+cats+data._highlightResult.title.value+'</a>' + snippet;
        },
        empty: function(data) {
